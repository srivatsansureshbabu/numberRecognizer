...
#         images = images.unsqueeze(1).to(device)  # shape becomes [batch_size, 1, 28, 28]
#         labels = labels.to(device)
#         outputs = model(images)
#         # Forward pass
#         loss = lossFunctionOutput(outputs, labels)
...
# # model.train()
# torch.save(model.state_dict(),"numberRecognizer.pth")
model.eval()
...
#         images = images.unsqueeze(1).to(device)  # add channel dim
#         labels = labels.to(device)
#         outputs = model(images)
...
# --- Now export to Core ML ---

# example_input = torch.rand(1, 1, 28, 28).to(device)  # dummy input for tracing
...
#     traced_model.cpu(),
#     inputs=[ct.ImageType(shape=example_input.shape,color_layout=ct.colorlayout.GRAYSCALE)]
# )

# # Save the Core ML model
# coreml_model.save("MNISTClassifier.mlpackage")
